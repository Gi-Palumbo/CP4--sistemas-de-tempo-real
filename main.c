...
// --- DADOS DO ALUNO ---
#define STUDENT_INFO "{Giovanna palumbo RM: 87767}"
...
void generator_task(void *pvParameters) {
    ...
    while (1) {
        ...
        if (xQueueSend(data_queue, &value_to_send, (TickType_t)0) == pdPASS) {
            ESP_LOGI(TAG, STUDENT_INFO " [TASK] Gerador: Valor %d enviado com sucesso.", value_to_send);
        } else {
            ESP_LOGW(TAG, STUDENT_INFO " [TASK] Gerador: AVISO - Fila cheia, valor %d descartado.", value_to_send);
        }
        ...
        if (xQueueReceive(data_queue, &received_value, pdMS_TO_TICKS(RECEIVE_TIMEOUT_MS)) == pdPASS) {
            ...
            if (temp_storage != NULL) {
                *temp_storage = received_value;
                ESP_LOGI(TAG, STUDENT_INFO " [TASK] Receptor: Valor %d recebido.", *temp_storage);
                free(temp_storage);
            } else {
                ESP_LOGE(TAG, STUDENT_INFO " [TASK] Receptor: ERRO - Falha ao alocar memória!");
                g_receiver_ok = false;
            }
        } else {
            timeout_count++;
            g_receiver_ok = false;
            if (timeout_count == 1) {
                ESP_LOGW(TAG, STUDENT_INFO " [TASK] Receptor: AVISO - Timeout 1 (Nenhum dado).");
            } else if (timeout_count == 2) {
                ESP_LOGE(TAG, STUDENT_INFO " [TASK] Receptor: ERRO - Timeout 2 (Sistema lento?).");
            } else {
                ESP_LOGE(TAG, STUDENT_INFO " [TASK] Receptor: ERRO FATAL - Timeout 3 (Gerador travou?).");
            }
        }
    }
}
...
void supervisor_task(void *pvParameters) {
    esp_task_wdt_add(NULL); // Adiciona a supervisão ao watchdog, pois é uma tarefa crítica
    while (1) {
        ...
        // Reinicia os flags para o próximo ciclo de verificação
        g_generator_ok = false;
        g_receiver_ok = false;
        ...
        printf("/********************************************************************/\n");
        printf("* Módulo Gerador...........: [ %-6s ]\n", gen_status_str);
        printf("* Módulo Receptor..........: [ %-6s ]\n", recv_status_str);
        printf("* Memória Livre (Heap).....: [ %-6ld bytes ]\n", esp_get_free_heap_size());
        printf("/********************************************************************/\n\n");
